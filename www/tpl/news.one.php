<main class="article wrap" >
	...
	<div class="topic">
	<?php echo $result['tagName'];?>
	</div>
	
	<img src="<?php echo $result['mainimg'];?>" alt="<?php echo $result['title'];?>" class="news-img-size">
	...
	<div>
		<? if($result['code']) {?><label>Дополнительно</label>
			<?=$result['code']?>
		<? } ?>
	</div>
	
	<? if($result2) {?><label>Дополнительные изображения</label><? } ?>
	<div class="min-images gallery">
		<?php if($result2) foreach($result2 as $value) { ?>
			<a href="<?php echo $value['url'];?>"><img src="<?php echo $value['url'];?>" alt="<?php echo $result['title'];?> <?php echo $value['id'];?>"></a>
		<?php } ?>
	</div>
</main>
<script>
